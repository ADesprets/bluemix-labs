...
| V1 | API Key | API Key |xref:#_protecting_an_api_with_an_api_key[here]
| V2 | API Key + Basic Authentication  | |xref:#_protecting_an_api_with_basic_authentication[here]
| V3 | Resource Owner Password Credentials Grant | Resource owner |xref:#_oauth_resource_owner_password_credentials_grant[here]
| V4 | Authorization Code grant + OIDC | Access code |xref:#_authorization_code_grant_and_oidc[here]
| V5 | Client Credentials grant | Application |xref:#_oauth_client_credentials_grant[here]
| V6 | External OAuth Provider | |xref:#_external_oauth_provider[here]
| V7 | Custom JWT Generate and Validate | |xref:#_custom_jwt_generate_and_validate[here]
| N/A | Using an OIDC Registry to protect the platform  | |xref:#_using_an_oidc_registry_to_protect_the_platform[here] 
|===
...
The important parts that we will change in the lab âre the base path, the description to indicate the security mechanism used, the security requirement and security scheme.
...
* Description: Using the native OAuth provider for Authorization grant
* Select OAuth2
* Select NativeProvider for the OAuth Provider
* Select Access code for the Flow
* Select details and openid for the scopes
